	...
	<script type="text/javascript">
		...
		function build_emps_table(result){
			...
			$.each(emps, function(index, item){
				var checkBox = $("<td><input type='chekcbox' class='check_item'/></td>");
		...
		function validate_add_form(){
			...
			if(!regName.test(empName)){
				//alert("用户名是2~5位中文或者6~16英文和数字的组合");
				show_validate_msg("#empName_add_input", "error", "用户名只能是2~5位中文或者6~16英文和数字的组合");
				return false;
			}else{
		...
		$("#emp_update_btn").click(function(){
			...
			if(!regEmail.test(regEmail)){
				//alert("邮箱格式不正确");
				show_validate_msg("#email_update_input", "error", "邮箱格式不正确，例：aa@qq.com");
				return false;
			}else{
